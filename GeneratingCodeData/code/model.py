...
class CheckPrepopMLP(nn.Module):
    def __init__(self, config):
        ...
        self.MLP_layers = nn.ModuleList([
            nn.Linear(hiddim, hiddim) for _ in range(num_layer)
        ])

        self.out_attr = nn.Linear(hiddim, 1)

    def forward(self, imgfeat):
        imgfeat = self.in_img(imgfeat)
        ...
        for layer in self.MLP_layers:
            ...
            h = F.relu(layer(h))
            h = h + h_res
        pred_pop = self.out_attr(h).squeeze()
        return pred_pop
...
class Noisy_AutoEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.config = config

        self.in_imgs = nn.Linear(config["img_dim"], 256)
        self.in_attr = nn.Linear(config["n_indim"], 256)
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Linear(256, 512),
            nn.ReLU(),
            nn.Linear(512, config["n_indim"]),
            nn.Tanh()
        )

    def forward(self, imgfeat, noisy_attr):
        img_h = self.in_imgs(imgfeat)
        attr_h = self.in_attr(noisy_attr)
        h = torch.cat([img_h, attr_h], dim=-1)
        h = self.encoder(h)
        pred_attr = self.decoder(h)
        return pred_attr
...
class DenoisingNet(nn.Module):
    def __init__(self, config):
        ...
        self.N_c_proj = nn.Linear((config["n_indim"])+2*config["LaPE_dim"], config["n_indim"]*2 + config["img_dim"])
        self.E_c_proj = nn.Linear(1, config["hiddim"])

        self.graph_transformer = GraphTransformer(config)

    def forward(self, inputs, dis, t, batchlization):
        # net
        n, e = inputs
        ...
        if len(t) == 1:
            ...
            _, t_emb_for_e = timestep_embedding(self.config, t, dim=self.config["hiddim"], max_period=self.config["T"], batchlization=batchlization)
            t_emb = t_emb_for_e

        dis_emb = dis.unsqueeze(-1)

        e_c = self.E_c_proj(dis_emb)
        e_c[batchlization == 0] = -2
...
class Diffusion(nn.Module):
    ...
    def q_sample(self, net, t, batchlization):
        ...
        if len(t) == 1:
            ...
            noisy_e = torch.zeros_like(noise_e).to(e.device)
            l, r = 0, 0
        ...
        return noisy_e, noise_e

    def loss(self, net, t, dis, batchlization):
        n, e = net
        noisy_e, noise_e = self.q_sample(net, t, batchlization)

        # inputs
        input_n = n
        input_e = noisy_e * batchlization
        inputs = (input_n, input_e)

        # model
        noise_e_hat = self.model(inputs, dis, t, batchlization)

        # loss
        noise_hat = noise_e_hat * batchlization
        noise     = noise_e    * batchlization
    ...
    def DDIM_sample(self, cur_e, t, t_pre, c):
        # condition content
        real_net, dis, batchlization = c # block because of batch lization
        real_n, real_e = real_net
        
        # inputs
        input_n = real_n
        input_e = cur_e * batchlization
        inputs = (input_n, input_e)
    ...
    def DDIM_sample_loop(self, shape_n, shape_e, c):
        # block matrix mask 
        _, _, batchlization = c # block because of batch lization
        # DDIM setting
        ...
        # denoising
        denoising_seq = [cur_e.to(torch.device("cpu"))]
        for t, t_pre in list(zip(reversed(sample_Ts), reversed(sample_Ts_pre))):
            ...
            cur_e = self.DDIM_sample(cur_e, t, t_pre, c)
            # save
            cur_e_cpu = cur_e.to(torch.device("cpu"))
            denoising_seq.append(cur_e_cpu)
