...
def JSD_in(a, b):
    ...
    a_dist = np.clip(a_dist, 1e-10, None)
    b_dist = np.clip(b_dist, 1e-10, None)
...
def vector_CosSim(pred, gt):
    if type(pred) == type(np.array([1, 1])):
        pred = torch.FloatTensor(pred)
    if type(gt) == type(np.array([1, 1])):
        gt = torch.FloatTensor(gt)
    pred_norm = (pred **2).sum(1).sqrt()
    gt_norm = (gt **2).sum(1).sqrt()
    cos_sim = (pred * gt).sum(1) / (pred_norm * gt_norm + 1e-20)
    return cos_sim